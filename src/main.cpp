...
const string strMessageMagic = "TrollPayCoin Signed Message:\n";
...
{
    const int nMinSubsidy = 0.1 * COIN;
	int64_t nSubsidy = 0 * COIN;
    if(nHeight == 1) {
    nSubsidy = 2000000 * COIN; // 2 mil premine for payouts & services rendered (blocks 2-4 .1 TPAY)
    } else if(nHeight >= 4 && nHeight <= 10004){ 
    nSubsidy = 200 * COIN;    //  			200 x 10,000 = 2 mil PoW 	 		 ( + Premine = 4 mil)
    } else if(nHeight >= 10005 && nHeight <= 20004){
    nSubsidy = 100 * COIN;    // 			100 x 10,000 = 1 mil PoW 			 ( + Prev Blocks = 5 mil)
    } else if(nHeight >= 20005 && nHeight <= 40004){
    nSubsidy = 50 * COIN;     // 			50 x 20,000 = 1 mil PoW				 ( + Prev Blocks = 6 mil)
    } else if(nHeight >= 40005 && nHeight <= 50004){ 
    nSubsidy = 100 * COIN;    // 			100 x 10,000 = 1 mil PoW			 ( + Prev Blocks = 7 mil)
    } else if(nHeight >= 50005 && nHeight <= 70004){ 
    nSubsidy = 50 * COIN;    // 			50 x 20,000 = 1 mil PoW 			 ( + Prev Blocks = 8 mil)
    } else if(nHeight >= 70005 && nHeight <= 80004){ 
    nSubsidy = 200 * COIN; // 				200 x 10,000 = 2 mil PoW			 ( + Prev Blocks = 10 mil)
    }
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("trollpaycoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TrollPayCoin
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TrollPayCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Top US intelligence official: I submitted my resignation - CNN.COM TrollPay.com";
        CTransaction txNew;
        txNew.nTime = 1479395605;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0xfffac93f56c0e50dd592171465b9af87120412f12e876ae1405a530623c865af"));
