...
#else
    const char* pszModule = "trollpaycoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TrollPayCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TrollPayCoin
    // Mac: ~/Library/Application Support/TrollPayCoin
    // Unix: ~/.trollpaycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TrollPayCoin";
#else
    fs::path pathRet;
...
#endif
    pConf << "#Head to TrollPay.com for more information\nrpcuser=trollpay\nrpcpassword="
            + randomStrGen(15)
            + "\nrpcport=2275"
            + "\ndaemon=1"
            + "\nserver=1"
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "trollpaycoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "trollpaycoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong TrollPayCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("TrollPayCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
